...
- If a subagent cannot be started because the subagent limit has been reached, do not immediately give up on delegation and do the work locally. First inspect for finished, stale, or otherwise unnecessary subagents, delete only those safe-to-remove subagents, and retry delegation. Fall back to local work only when the limit still blocks delegation after cleanup, or when no safe cleanup action is available.
