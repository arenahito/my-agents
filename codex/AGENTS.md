...
- The main agent should delegate well-scoped work to subagents by default, provided delegation does not materially reduce quality. Handle it directly when this quality condition cannot be met or delegation overhead clearly outweighs the benefit. Briefly explain direct handling only when the user expects delegation. Assess related small tasks together rather than treating each as an exception.
...
- When choosing a model under these defaults, use Astra only for design tasks. This restriction also applies when escalating to a stronger model or replacing an unavailable model.
...
- When choosing settings under these defaults, select the model and effort explicitly instead of unintentionally inheriting an expensive orchestrator configuration. If capability limits become evident, pass the useful findings to a stronger model allowed by the rules above rather than repeatedly retrying with the same model.
